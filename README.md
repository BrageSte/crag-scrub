...
1. Install Python 3.11+ (macOS already includes Python once you install Xcode Command Line Tools). Then open Terminal, `cd` into the unpacked repo folder, and install dependencies:
   ```bash
   python -m pip install -e .
   ```
   This step prevents `ModuleNotFoundError: cragscrub` when starting the app.

2. Start the GUI:
   ```bash
   cragscrub-gui
   # or: python -m cragscrub.gui
   # or double-click run-gui.command after running: chmod +x run-gui.command
   ```
...
4. Watch the log panel for progress; when done, the NDJSON is ready for your app. Filter-failed crags remain with `effective_filter_passed=false` so you can audit them separately.

**Tip (macOS double-click flow):** if you prefer double-clicking `run-gui.command`, first open Terminal in the repo folder and run `chmod +x run-gui.command && ./run-gui.command`. The script will reuse your default Python and the installed dependencies from step 1.
